...
TextPairExample = namedtuple("TextPairExample", ["id", "claim", "evidence", "label"])
...
def main():
    ...
    parser.add_argument("--sorted", action="store_true",
                        help='Sort the data so most batches have the same input length,'
                             ...
                             'its off by default.')
    parser.add_argument("--which_bias", choices=["fever_claim_only", "fever_claim_only_bow",
    ...
    train_examples = None
    if args.do_train:
        train_examples = load_fever(split="train", sample=2000 if args.debug else None)
    ...
    if args.do_train:
        ...
        if args.mode != "none":
            ...
            train_features = [x for x in train_features if str(x.example_id) in bias_map]
            ...
            for fe in train_features:
                fe.teacher_probs = np.array(teacher_probs_map[str(fe.example_id)]).astype(np.float32)
